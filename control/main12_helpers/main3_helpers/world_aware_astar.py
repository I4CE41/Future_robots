...
Drop-in replacement for AStarPlanner that handles negative world coordinates.
The original AStarPlanner clipped negative-X / negative-Y obstacles and goals
into a 0..10 grid, silently dropping them. This version centers the grid on
the origin and uses explicit world<->grid helpers.
...
class WorldAwareAStar:
    ...
    def plan(self, start_xy, goal_xy, obstacles):
        if not self._in_bounds(*goal_xy) and not self._warned_oob:
            print(f"[WorldAwareAStar] warning: goal ({goal_xy[0]:.2f},{goal_xy[1]:.2f}) "
                  f"outside grid [{self.x_min},{self.x_max}]; clamped.")
            self._warned_oob = True

        grid = np.zeros((self.rows, self.cols), dtype=np.uint8)

        # Inflate every obstacle by robot_radius + its own radius
        for obs in obstacles:
            ...
            # Skip if obstacle centre is way out of bounds (saves time)
        ...
        # If start happens to land in an inflated cell, drill it open so we can leave
        if grid[sr, sc] == 1:
            grid[max(0, sr - 1):min(self.rows, sr + 2),
                 max(0, sc - 1):min(self.cols, sc + 2)] = 0

        # If goal is in an inflated cell, do the same (so we don't return [goal_xy])
        if grid[gr, gc] == 1:
