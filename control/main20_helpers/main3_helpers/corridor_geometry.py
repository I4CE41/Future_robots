...
class CorridorGeometryExtractor:
    ...
    def _open_state(self, robot_xy, goal_xy=None, obstacles=None):
        ...
        return CorridorState(
            ...
            width_profile=[width],
            is_narrow=narrow,  # consistent with extract() — True for dead_end/tight/narrow
    ...
    def extract(self, state, goal_xy, path=None, obstacles=None):
        ...
        # Global direction: used as fallback and for alignment/angle computation.
        ...
        for i, (lx, ly) in enumerate(local_path):
            wx, wy = rx + lx, ry + ly
            # --- BUG FIX: use LOCAL path direction at each waypoint, not the
            # single global endpoint direction.  For curved / slalom paths the
            # global direction gives a diagonal cross-section that over-reports
            # corridor width, masking the actual pinch the robot is about to
            # enter.  Using the forward segment direction at each sample point
            # gives the true perpendicular clearance.
            if i + 1 < len(local_path):
                nlx, nly = local_path[i + 1]
                seg_dir = (nlx - lx, nly - ly)
            elif i > 0:
                plx, ply = local_path[i - 1]
                seg_dir = (lx - plx, ly - ply)
            else:
                seg_dir = direction
            use_dir = seg_dir if math.hypot(seg_dir[0], seg_dir[1]) > 1e-6 else direction
            width, left, right, offset = self._cross_section(
                (wx, wy), use_dir, obstacles)
            widths.append(width)
            lefts.append(left)
            rights.append(right)
