...
class EscalatingRecovery:
    """Reverse-and-reface recovery that ESCALATES when it keeps firing inside the
    same mission phase.

    The shared main9 RecoveryAction always does the same 0.6 s nudge. On the
    clustered slalom this livelocks: the robot backs out 0.6 s, drives into the
    exact same trap, the watchdog re-fires, and the identical weak kick repeats
    for the full 240 s budget (~45 failed kicks -> timeout, E>150 Wh). This is
    the dominant failure mode across all variants, and it is worst for the fast
    variants (speed_only) that wedge deepest.

    Fix: each consecutive trigger WITHOUT phase progress backs out longer and
    turns harder, and at high levels alternates turn direction to break the
    symmetry that keeps re-entering the same gap. reset_escalation() is called on
    every mission phase change (genuine progress), so seeds that are NOT stuck
    behave exactly like the 0.6 s baseline and cannot regress. Drop-in compatible
    with RecoveryAction: .active, trigger(), step()."""

    def __init__(self, dt=0.1, reverse_s=0.6, v_back=-0.25):
        self.dt = float(dt)
        self.base_reverse_steps = max(1, int(reverse_s / dt))
        self.v_back = float(v_back)
        self.t = 0
        self.active = False
        self.turn_dir = 1.0
        self.level = 0
        self.reverse_steps = self.base_reverse_steps

    def reset_escalation(self):
        """Called on mission phase change: the robot made real progress, so the
        next recovery starts fresh at the gentle baseline."""
        self.level = 0

    def trigger(self, state, goal_xy):
        self.active = True
        self.t = 0
        # Escalate: level 1 -> 0.6 s, 2 -> ~1.2 s, 3 -> ~1.8 s, capped at 4.
        self.level = min(self.level + 1, 4)
        self.reverse_steps = int(self.base_reverse_steps * (1.0 + 0.9 * (self.level - 1)))
        alpha = wrap_angle(math.atan2(goal_xy[1] - state["y"],
                                      goal_xy[0] - state["x"]) - state["theta"])
        base_dir = 1.0 if alpha >= 0 else -1.0
        # Levels 1-2 turn toward the goal; levels 3+ alternate to try the other
        # side of the obstacle that keeps trapping us.
        if self.level >= 3 and (self.level % 2 == 1):
            self.turn_dir = -base_dir
        else:
            self.turn_dir = base_dir
    ...
    def step(self):
        ...
        if self.t <= self.reverse_steps:
            # Harder reverse and turn as level climbs (full reverse speed and a
            # stronger yaw rate at high levels to physically clear the wedge).
            v_scale = 1.0 + 0.25 * (self.level - 1)
            w_mag = min(1.4, 0.8 + 0.2 * (self.level - 1))
            return self.v_back * v_scale, w_mag * self.turn_dir, False
        self.active = False
        return 0.0, 0.0, True


from robot_rescue.control.main14_helpers.nmpc_controller import NMPCController
from robot_rescue.control.main14_helpers.safety import SafetyFilter
from robot_rescue.control.main14_helpers.main3_helpers.shims import apply_shims
from robot_rescue.control.main14_helpers.main3_helpers.dynamic_tracker import DynamicTracker
from robot_rescue.control.main14_helpers.main3_helpers.world_aware_astar import WorldAwareAStar
from robot_rescue.control.main14_helpers.main3_helpers.energy_paper import PhysicsEnergyManager
from robot_rescue.control.main14_helpers.main3_helpers.perception_fusion import PerceptionFusion
from robot_rescue.control.main14_helpers.main3_helpers.path_clearance import (
    CorridorPredictor,
...
from robot_rescue.control.main14_helpers.main3_helpers.corridor_geometry import (
...
class EnhancedCorridorWarehouseMission(CorridorWarehouseMission):
    """main12 layout: a slalom placed ON the pick->place path (not beside it).

    main10 offsets the three obstacles to one side (lat -0.35/-0.95/-0.35), so
    the robot can largely skirt them along the open side -- the avoidance
    methods barely engage. Here we straddle the centerline with alternating
    offsets (+0.28/-0.30/+0.28), so each obstacle sits directly on the straight
    pick->place line and the robot is forced to weave left-right-left through
    ~0.57 m free gaps. This makes the CAPP/DWA/CBF avoidance visibly active
    while remaining solvable (gap >> robot+obstacle radius). main10 is untouched.
    """

    # On-path slalom: (along-track fraction, lateral offset in perp frame).
    # Alternating signs straddle the centerline. perp points toward -y here, so
    # the obstacle's perpendicular distance from the straight line is |lat|.
    #
    # GEOMETRY CONSTRAINT (verified): obstacle radius 0.22 m, robot radius
    # ~0.30 m, so a threadable gap needs centre-to-centre spacing >= 0.44 +
    # 0.60 = ~1.04 m, with margin ~1.20 m. The earlier +0.28/-0.30/+0.28 layout
    # packed obstacles 1.01 m apart -> 0.57 m surface gap < 0.60 m robot width
    # -> physically impossible, robot jammed and timed out. This layout spreads
    # the along-track fractions to 0.26/0.52/0.78 (centre spacing 1.25 m -> 0.81 m
    # surface gap, comfortably threadable) while keeping a +-0.18 m straddle so
    # each obstacle still sits on the path and forces a genuine steer-around
    # (robot radius 0.30 > 0.18 offset).
    _ONPATH_SLALOM = [(0.26, +0.18), (0.52, -0.18), (0.78, +0.18)]
    ...
    def _spawn_corridor_field(self):
        ...
        if dist < 2.0:
            ...
            def clear(x, y, m=0.20):
                return all(math.hypot(x - cx, y - cy) >= cr + m
                           for cx, cy, cr in hard)

            placed = 0
            for along, lat in self._ONPATH_SLALOM:
            ...
            self.n_dynamic = 0
            print(f"[Corridor] {placed} on-path slalom + 0 dynamic obstacles "
                  f"(main12 FIXED mode, straddling pick->place line).")
        try:
            p1 = np.asarray(self.pick_pos, float)
            p2 = np.asarray(self.place_pos, float)
            vec = p2 - p1
...
def run_mission(args):
    ...
    recovery = EscalatingRecovery(dt=nmpc_dt)
    ...
    wedge_escapes = 0
    # Anti-loop reroute: track where escapes/recoveries keep firing. If the robot
    # escapes the SAME spot more than `escape_loop_limit` times, the gap it keeps
    # retrying is effectively infeasible from this approach -- backing out and
    # replanning into the same gap is an infinite loop (observed: 21-35 escapes,
    # still timed out). When the limit trips we blacklist a WIDE radius around
    # that spot and force a conservative wide-clearance replan so A* is pushed to
    # a different corridor instead of the jammed one.
    escape_anchor = None          # (x, y) of the current repeated-stall cluster
    escape_anchor_count = 0       # stalls near that anchor
    escape_loop_limit = 2         # trips -> force wide reroute (Option B: commit
                                  # to the detour after only 2 stalls, ~10 s, so
                                  # the robot routes AROUND the cluster instead of
                                  # brute-forcing the jammed gap for 200+ s)
    force_wide_reroute = 0        # ticks remaining to bias A* toward wide gaps
    ...
    try:
        for step in range(args.max_steps):
            ...
            if mission.phase != prev_phase:
                ...
                collision_hits = 0
                escape_anchor = None
                escape_anchor_count = 0
                force_wide_reroute = 0
                recovery.reset_escalation()
            ...
            if force_wide_reroute > 0:
                force_wide_reroute -= 1
            ...
            if step % ctrl_every == 0:
                ...
                corridor_state = corridor_extractor.extract(
                    ...
                    augmented_obstacles)
                if corridor_state.is_narrow or corridor_state.is_tight or corridor_state.is_dead_end:
                    dynamic_sweep = predicted_obstacle_sweep(tracker, sim_time,
                                                             horizon_s=2.5,
                                                             dt_sample=0.05,
                                                             inflate=0.16)
                ...
                if mission.is_arm_phase:
                    ...
                    try:
                        ...
                        u, _, _ = nmpc.solve(state["full_q"], goal, obstacles, soc,
                                             u_prev=nmpc_prev,
                    ...
                    if need:
                        ...
                        # When escaping a jammed gap, bias BOTH plans toward wider
                        # clearance so A* is pushed out of the infeasible corridor.
                        wide = force_wide_reroute > 0
                        plan_a, info_a = plan_with_clearance(
                            astar, (rx, ry), goal_xy, augmented_obstacles,
                            min_clearance=0.40 if wide else 0.30,
                            gap_min=1.15 if wide else 0.95,
                            max_attempts=8 if wide else 6,
                            inflation_step=0.30 if wide else 0.25,
                        )
                        candidate_paths.append(("clear", densify(plan_a if plan_a else [(rx, ry), goal_xy], 0.15)))

                        plan_b, info_b = plan_with_clearance(
                            astar, (rx, ry), goal_xy, augmented_obstacles,
                            min_clearance=0.46 if wide else 0.38,
                            gap_min=1.25 if wide else 1.05,
                            max_attempts=6 if wide else 4,
                            inflation_step=0.30,
                        )
                    ...
                    geometric_wedge = (
                        ...
                        and (corridor_state.is_tight or corridor_state.is_dead_end)
                    )
                    wedge_now = (
                        geometric_wedge
                        and is_wedged(perc)
                    ...
                    if wedge_now:
                        ...
                        # Anti-loop: if escapes keep firing within ~0.5 m of the
                        # same anchor, the gap is infeasible from here. Count
                        # them; on the Nth, force a wide reroute (below).
                        if (escape_anchor is not None
                                and math.hypot(rx - escape_anchor[0],
                                               ry - escape_anchor[1]) < 0.5):
                            escape_anchor_count += 1
                        else:
                            escape_anchor = (rx, ry)
                            escape_anchor_count = 1
                        if escape_anchor_count >= escape_loop_limit:
                            force_wide_reroute = int(4.0 / nmpc_dt)
                            escape_anchor_count = 0
                            if args.debug_nav:
                                print(f"[dbg] s{step:5d} ESCAPE-LOOP at "
                                      f"({rx:+.2f},{ry:+.2f}) -> forcing wide "
                                      f"reroute", flush=True)
                        ...
                        # Option B root-cause fix. The watchdog -- not the wedge
                        # branch -- is what actually fires in the slalom (the
                        # geometric wedge test never trips here, so wedge_escapes
                        # stays 0). Previously this branch only reversed and then
                        # drove straight back into the SAME gap, looping until the
                        # 240 s timeout. Instead we now COUNT repeated stalls near
                        # the same spot and, after escape_loop_limit of them, force
                        # a wide reroute so A* routes AROUND the obstacle cluster
                        # through the open space above/below the line. This fixes
                        # the loop at its source rather than escalating the kick.
                        recovery.trigger(state, goal_xy)
                        corridor_blacklist.mark_bad(corridor_report)
                        ...
                        if (escape_anchor is not None
                                and math.hypot(rx - escape_anchor[0],
                                               ry - escape_anchor[1]) < 0.6):
                            escape_anchor_count += 1
                        else:
                            escape_anchor = (rx, ry)
                            escape_anchor_count = 1
                        if escape_anchor_count >= escape_loop_limit:
                            force_wide_reroute = int(6.0 / nmpc_dt)
                            escape_anchor_count = 0
                            if args.debug_nav:
                                print(f"[dbg] s{step:5d} STALL-LOOP at "
                                      f"({rx:+.2f},{ry:+.2f}) -> wide reroute "
                                      f"around cluster", flush=True)
                        bv, bw, _ = recovery.step()
                        final_control = np.array([bv, bw, 0, 0, 0])
                    else:
                        dock_stall = 0
                    ...
                    # --- stall diagnostics (--debug-nav) --------------------
                    # Prints the exact inputs to the escape/recovery gate ONLY
                    # when the robot is both near an obstacle (<0.8 m) and barely
                    # moving (|v|<0.08 m/s) -- i.e. the moments where it creeps
                    # instead of escaping. This reveals why _corridor_escape_control
                    # never fires in the slalom (wedge_escapes stays 0): we can see
                    # the corridor classification, the wedge-test components, and
                    # which control branch actually drove the wheels.
                    if args.debug_nav:
                        near = min((math.hypot(ox - rx, oy - ry) - r
                                    for ox, oy, r in augmented_obstacles),
                                   default=99.0)
                        if near < 0.80 and abs(float(final_control[0])) < 0.08:
                            if wedge_now:
                                branch = "ESCAPE"
                            elif recovery.active:
                                branch = "recovery"
                                ...
                                branch = "pursuit"
                            print(f"[dbg] s{step:5d} {mission.phase:12s} "
                                  f"near={near:+.2f} cls={corridor_class:11s} "
                                  f"minW={getattr(corridor_state,'min_width',float('inf')):.2f} "
                                  f"F={sensor_front:.2f} L={sensor_left:.2f} R={sensor_right:.2f} "
                                  f"wedged={is_wedged(perc)} geoW={geometric_wedge} "
                                  f"-> {branch:8s} v={float(final_control[0]):+.3f} "
                                  f"w={float(final_control[1]):+.3f}", flush=True)
...
def parse_args():
    ...
    ap.add_argument("--debug-nav", action="store_true", default=False,
                    help="Print per-tick stall diagnostics (corridor class, "
                         "clearances, wedge test, active control branch) whenever "
                         "the robot is near an obstacle and barely moving. Use to "
                         "diagnose slalom creep/wedging.")
