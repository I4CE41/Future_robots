...
_HERE = os.path.dirname(os.path.abspath(__file__))   # robot_rescue/Main12_fix
_PKG = os.path.dirname(_HERE)                          # robot_rescue
_ROOT = os.path.dirname(_PKG)                          # lastry_improved
# Isolated memory so the fixed variant neither reads nor pollutes the original
# main12 path_memory.json.
PATH_MEMORY_FILE = os.path.join(_HERE, "Main12_fix_path_memory.json")
# _ROOT (lastry_improved) must come FIRST so bare `control` resolves to
# lastry_improved/control (where main9 expects nmpc_controller), not
# robot_rescue/control. _PKG (robot_rescue) is appended so `from main9 import`
# / `from main10 import` resolve without shadowing `control`. Mirrors the
# original main12.py path setup.
...
# --- Main12_fix patched components (corridor-stuck / wall-hit fixes) ---------
from robot_rescue.Main12_fix.Main12_helpers.safety_patched import PatchedSafetyFilter
from robot_rescue.Main12_fix.Main12_helpers.control_patched import (
    CorridorAwareWatchdog,
    WallAwareRecovery,
)
from robot_rescue.Main12_fix.Main12_helpers.planning_patched import (
    plan_with_clearance_safe,
    reject_colliding,
)
...
def run_mission(args):
    ...
    safety = PatchedSafetyFilter(config)
    ...
    conflict_predictor = DynamicConflictPredictor(lookahead_path_m=2.2,
                                                  ...
                                                  dt=nmpc_dt)
    watchdog = CorridorAwareWatchdog(window_s=3.0, min_gain=0.15, dt=nmpc_dt)
    recovery = WallAwareRecovery(dt=nmpc_dt)
    ...
    collision_hits = 0
    no_path_ticks = 0   # ticks where no collision-free candidate path existed
    failure = None
    ...
    try:
        for step in range(args.max_steps):
            ...
            if step % ctrl_every == 0:
                ...
                # CAUSE-3 FIX: keep the FULL obstacle set for collision-awareness
                # (costmap, safety filter, reactive avoider, candidate rejection)
                # so the robot can still "see" a wall it is hugging. Only the A*
                # PLANNER gets the near-robot-filtered list, because A* cannot
                # route out of a cell it is standing inside. The original code
                # filtered the single shared list, which blinded everything to
                # close walls and let the straight-line fallback cut through them.
                obstacles_plan = [(ox, oy, r) for ox, oy, r in obstacles
                ...
                augmented_plan = obstacles_plan + dynamic_sweep
                ...
                if mission.is_arm_phase:
                    ...
                    if need:
                        # CAUSE-2 FIX: never inject an unchecked straight line.
                        # Each candidate is validated against the FULL obstacle
                        # set (augmented_obstacles) via reject_colliding before
                        # it is allowed to compete; the safe planner returns None
                        # instead of a fake [(rx,ry),goal] path on failure.
                        candidate_paths = []
                        if los_safe(state, goal_xy, augmented_obstacles,
                                    robot_radius=0.3, safety=0.45):
                            direct_path = densify([(rx, ry), goal_xy], 0.15)
                            if reject_colliding(direct_path, augmented_obstacles, min_ok=0.05):
                                candidate_paths.append(("direct", direct_path))

                        # A* plans on the near-robot-filtered list (augmented_plan)
                        # but candidates are collision-checked on the full set.
                        plan_a, info_a = plan_with_clearance_safe(
                            astar, (rx, ry), goal_xy, augmented_plan,
                            min_clearance=0.30, gap_min=0.95,
                            max_attempts=6, inflation_step=0.25,
                        )
                        if plan_a and len(plan_a) >= 2:
                            cand_a = densify(plan_a, 0.15)
                            if reject_colliding(cand_a, augmented_obstacles, min_ok=0.05):
                                candidate_paths.append(("clear", cand_a))

                        plan_b, info_b = plan_with_clearance_safe(
                            astar, (rx, ry), goal_xy, augmented_plan,
                            min_clearance=0.38, gap_min=1.05,
                            max_attempts=4, inflation_step=0.30,
                        )
                        if plan_b and len(plan_b) >= 2:
                            cand_b = densify(plan_b, 0.15)
                            if reject_colliding(cand_b, augmented_obstacles, min_ok=0.05):
                                candidate_paths.append(("conservative", cand_b))

                        if not candidate_paths:
                            # No collision-free path this tick. Do NOT fabricate a
                            # straight line into the wall. Keep the last known-good
                            # path if it is still safe; otherwise clear it so the
                            # controller stops-and-reorients instead of charging in.
                            if not (current_path and
                                    reject_colliding(current_path, augmented_obstacles, min_ok=0.05)):
                                current_path = []
                            no_path_ticks += 1
                            last_astar = step
                        else:
                    ...
                    if wedge_now:
                        ...
                        if dock_stall > dock_stall_limit:
                            recovery.trigger(state, goal_xy, augmented_obstacles)
                            current_path = []
                            dock_stall = 0
                            bv, bw, _ = recovery.step()
                            final_control = np.array([bv, bw, 0, 0, 0])
                    elif watchdog.update(state, goal_xy,
                                         float(getattr(corridor_state, "velocity_scale", 1.0))):
                        # CAUSE-5 FIX: recovery now checks rear clearance and
                        # pivots instead of reversing when a wall is behind.
                        recovery.trigger(state, goal_xy, augmented_obstacles)
                        corridor_blacklist.mark_bad(corridor_report)
                        ...
                        # CAUSE-6 FIX: the original code MULTIPLIED three throttles
                        # (clearance ~0.55 x corridor ~0.25 x front 0.75 ~= 0.10),
                        # crushing v to a near-stop crawl that reads as "stuck".
                        # Take the single most-restrictive scale instead of stacking
                        # them, then hold a forward creep whenever the path ahead is
                        # actually clear so the robot keeps threading the corridor.
                        scale_clear = _clearance_speed_scale(current_min_clearance)
                        scale_corr = float(getattr(corridor_state, "velocity_scale", 1.0))
                        scale_front = 0.75 if sensor_front < 0.55 else 1.0
                        v *= min(scale_clear, scale_corr, scale_front)
                        lookahead_clear = lookahead_min_clearance(
                            current_path, augmented_obstacles, (rx, ry), lookahead_m=1.2) \
                            if current_path else float("inf")
                        if v > 0.0 and lookahead_clear > 0.30 and sensor_front > 0.30:
                            v = max(v, 0.18)
...
def print_summary(m):
    print("\n" + "=" * 60)
    print(" Chapter-4 Warehouse Mission Summary (main12-fix)")
    print("=" * 60)
...
def run_batch(args):
    ...
    print("\n" + "=" * 64)
    print(f" main12-fix batch: variant={args.variant}  {len(ok)}/{n} success "
...
def parse_args():
    ap = argparse.ArgumentParser(description="main12-fix corridor-robust benchmark")
    ...
    ap.add_argument("--batch-out", type=str, default="Main12_fix_batch.csv")
